...
request.onload = function () {
    ...
    if (request.status >= 200 && request.status < 400) {
        ...
        for (var i = 0; i < data.confirmed.locations.length; i++) {
            if (data.confirmed.locations[i].country != undefined) {
                if (dict[data.confirmed.locations[i].country] != null) {
                    dict[data.confirmed.locations[i].country][0] = dict[data.confirmed.locations[i].country][0] + data.confirmed.locations[i].latest
                    dict[data.confirmed.locations[i].country][1] = dict[data.confirmed.locations[i].country][1] + data.deaths.locations[i].latest
                    dict[data.confirmed.locations[i].country][2] = dict[data.confirmed.locations[i].country][2] + data.recovered.locations[i].latest

                } else {
                    dict[data.confirmed.locations[i].country] = [0, 0, 0]
                    if (data.confirmed)
                        dict[data.confirmed.locations[i].country][0] = data.confirmed.locations[i].latest
                    if (data.deaths)
                        dict[data.confirmed.locations[i].country][1] = data.deaths.locations[i].latest
                    if (data.recovered)
                        dict[data.confirmed.locations[i].country][2] = data.recovered.locations[i].latest
    ...
    for (var i = 0; i < data.confirmed.locations.length; i++) {
        ...
        if (data.confirmed.locations[i].country == document.getElementById('dropdownCountry').value || document.getElementById('dropdownCountry').value.length == 0 || document.getElementById('dropdownCountry').value == 'Global') {
            ...
            for (var h in data.confirmed.locations[i].history) {
                var dateT = h.split('/')
                dictConverTime[new Date(h).getTime()] = dateT[1] + '/' + dateT[0] + '/' + dateT[2]
                temp2Confirmed[new Date(h).getTime()] = data.confirmed.locations[i].history[h]
                temp2Deaths[new Date(h).getTime()] = data.deaths.locations[i].history[h]
                temp2Recovered[new Date(h).getTime()] = data.recovered.locations[i].history[h]
            }
